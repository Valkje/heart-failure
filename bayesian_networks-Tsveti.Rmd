...
Barely any difference :(, does not seem very useful
...
#Case controls with propensity scores

```{r}
dat_cc_prop_scores <- dat_scaled
dat_cc_prop_scores$ps <- ps

```

```{r}
# Sort data by propensity scores
dat_cc_prop_scores <- dat_cc_prop_scores[order(dat_cc_prop_scores$ps),]

matchit <- matchit(Event ~ ps, data = dat_cc_prop_scores, method = "nearest", replace = FALSE)
...
```{r}
m_matched <- lm(Event ~ Creatinine, data = matched_data)
coef_matched <- coef(m_matched)

print(coef_matched)
```
```{r}

m_population <- lm(Event ~ Creatinine, data = dat_cc_prop_scores)
coef_population <- coef(m_population)

print(coef_population)
```

```{r}
# Plot the coefficient for Creatinine
plot(coef_matched['Creatinine'], 2, xlim=c(0,0.3), ylim=c(0.5,2.5), pch=19,
     yaxt="n", ylab="", bty="n" )

ci_matched <- confint(m_matched)['Creatinine',]
segments(x0=ci_matched[1], x1=ci_matched[2], y0=2, y1=2)

# Plot the population
ci_population <- confint(m_population)['Creatinine',]

points(coef_population['Creatinine'], 1, pch=19, col=2)
segments(x0=ci_population[1], x1=ci_population[2], y0=1, y1=1)

legend("topright", legend=c("Matched Sample", "Population"),
       pch=19, col=1:2)

```
Effect is slightly stronger for population than in the sample, aka matched sample suggests that creatinine has a smaller effect on Event than the population would suggest (this is also backed by our covariate adjustment)
```{r}
m_matched <- glm(Event ~ Creatinine, data = matched_data, family="binomial")
coef_matched <- coef(m_matched)
...
```{r}
m_population <- glm(Event ~ Creatinine, data = dat_cc_prop_scores, family="binomial")
coef_population <- coef(m_population)
...
```{r}
plot(coef_matched['Creatinine'], 2, xlim=c(0,1.5), ylim=c(0.5,2.5), pch=19,
     yaxt="n", ylab="", bty="n" )
...
coef(m_matched)
```
I dont really see a difference apart from the scale 
...
```{r}
# Assuming df is your dataframe and col_to_move is the name of the column you want to move to the end
